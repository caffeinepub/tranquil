...
export function SoundTile({ id, title, emoji, isPlaying, onToggle }: SoundTileProps) {
  return (
    <button
      onClick={() => onToggle(id)}
      className={`
    ...
    >
      ...
      <span className="text-xs font-semibold text-foreground text-center leading-tight">{title}</span>
      ...
      {!isPlaying && (
        <div className="flex items-end gap-0.5 h-5">
          {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className="w-1 h-1 rounded-full bg-muted-foreground opacity-40" />
          ))}
        </div>
      )}
    </button>
