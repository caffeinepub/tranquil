...
import { useCalmRoutine, CalmPhase } from '../hooks/useCalmRoutine';
...
const PHASE_CONFIG: Record<CalmPhase, { title: string; subtitle: string; emoji: string; bg: string }> = {
  breathing: {
    title: 'Breathe',
    subtitle: 'Inhale slowly for 4 counts, exhale for 6 counts. Let your body relax.',
    emoji: '🌬️',
    bg: 'from-teal-400/20 to-cyan-400/20',
  },
  bodyScan: {
    title: 'Body Scan',
    subtitle: 'Close your eyes. Relax your shoulders, unclench your jaw, soften your hands.',
    emoji: '🧘',
    bg: 'from-violet-400/20 to-purple-400/20',
  },
  affirmation: {
    title: 'You Are Safe',
    subtitle: 'This moment will pass. You are stronger than you think. You are doing great.',
    emoji: '💙',
    bg: 'from-blue-400/20 to-indigo-400/20',
  },
  complete: {
    title: 'Well Done',
    subtitle: 'You completed your calm routine. Take a moment to appreciate yourself.',
    emoji: '✨',
...
export function EmergencyCalmOverlay({ onClose }: EmergencyCalmOverlayProps) {
  ...
  useEffect(() => {
    if (state.isComplete) {
      addBreathingSession.mutate({ technique: 'Emergency Calm', durationSeconds: BigInt(60) });
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [state.isComplete]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/95 backdrop-blur-xl">
      <div className={`absolute inset-0 bg-gradient-to-br ${config.bg} transition-all duration-1000`} />
      ...
      <div className="relative w-full max-w-sm mx-4 flex flex-col items-center gap-6 text-center">
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          {!state.isComplete && <span className="text-xs text-muted-foreground">seconds remaining</span>}
        </div>
        ...
        <div className="flex gap-2">
          {(['breathing', 'bodyScan', 'affirmation'] as CalmPhase[]).map((p, i) => (
            <div
              key={p}
        ...
        <div className="animate-fade-in-up">
          ...
          <h2 className="text-3xl font-bold font-display text-foreground mb-3">{config.title}</h2>
          <p className="text-muted-foreground leading-relaxed text-sm">{config.subtitle}</p>
        </div>
