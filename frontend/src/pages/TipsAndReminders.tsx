...
import { useTips, useReminderPrefs, useUpdateReminderPrefs } from '../hooks/useQueries';
...
export function TipsAndReminders() {
  ...
  return (
    <div className="px-4 py-5 space-y-6 animate-fade-in-up">
      <div>
        <h1 className="text-2xl font-bold font-display text-foreground">AI Tips 💡</h1>
        <p className="text-sm text-muted-foreground mt-1">Personalized wellness guidance for you</p>
      </div>

      {/* Tips Section */}
      <div className="space-y-3">
        <h2 className="text-sm font-bold text-foreground uppercase tracking-wider">Today's Wellness Tips</h2>
      ...
      <div className="p-4 rounded-2xl bg-gradient-to-br from-primary/10 to-secondary/10 border border-primary/20">
        <h3 className="text-sm font-bold text-foreground mb-2">🌟 Daily Insight</h3>
        <p className="text-xs text-muted-foreground leading-relaxed">
          Consistent small habits — like drinking water, taking short breaks, and practicing breathing —
          can reduce stress levels by up to 40% over time. Keep going!
        </p>
      </div>
    </div>
  );
