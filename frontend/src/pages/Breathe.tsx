...
import { BREATHING_TECHNIQUES, useBreathingTimer, BreathingTechnique } from '../hooks/useBreathingTimer';
...
export function Breathe() {
  ...
  const { state, start, pause, resume, reset } = useBreathingTimer(selectedTechnique);
  ...
  return (
    <div className="px-4 py-5 space-y-6 animate-fade-in-up">
      <div>
        <h1 className="text-2xl font-bold font-display text-foreground">Breathe 🌬️</h1>
        <p className="text-sm text-muted-foreground mt-1">Choose a technique and find your calm</p>
      </div>
          ...
          {BREATHING_TECHNIQUES.map(technique => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-foreground text-sm">{technique.name}</h3>
                <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">{technique.description}</p>
      ...
      {selectedTechnique && (
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="text-center">
            ...
            <h2 className="text-lg font-bold font-display text-foreground mt-1">{selectedTechnique.name}</h2>
            {!state.isComplete && (
          ...
          <div className="relative flex items-center justify-center" style={{ height: 260 }}>
            <BreathingCircle phase={state.phase} phaseProgress={state.phaseProgress} size={220} />
          </div>
          ...
          {!state.isComplete ? (
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="text-sm text-muted-foreground text-center">
                Duration: {Math.floor(state.elapsedSeconds / 60)}m {state.elapsedSeconds % 60}s
              </p>
              <button
