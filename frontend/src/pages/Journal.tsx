...
export function Journal() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await addMoodEntry.mutateAsync({ mood: selectedMood, note: note.trim() || null });
  ...
  const getMoodLabel = (mood: string) => MOODS.find(m => m.value === mood)?.label ?? mood;
  ...
  return (
    <div className="px-4 py-5 space-y-6 animate-fade-in-up">
      <div>
        <h1 className="text-2xl font-bold font-display text-foreground">Mood Journal 📔</h1>
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-bold text-foreground uppercase tracking-wider">This Week</h2>
        ...
        {entries && entries.length > 0 && (
          <div className="space-y-2">
            {[...entries].reverse().map((entry, i) => (
              <div key={i} className="flex items-start gap-3 p-4 bg-card rounded-2xl border border-border">
                <span className="text-2xl flex-shrink-0">{getMoodEmoji(entry.mood)}</span>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-semibold text-foreground">{getMoodLabel(entry.mood)}</span>
