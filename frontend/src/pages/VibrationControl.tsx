...
const PATTERNS = [
  { id: 'gentle-pulse', label: 'Gentle Pulse', emoji: '💓', description: 'Soft rhythmic pulses for relaxation' },
  { id: 'rhythmic-tap', label: 'Rhythmic Tap', emoji: '🥁', description: 'Steady tapping to ground your focus' },
  { id: 'deep-wave', label: 'Deep Wave', emoji: '🌊', description: 'Long deep waves for stress release' },
];

export function VibrationControl() {
  const [selectedPattern, setSelectedPattern] = useState<string>('gentle-pulse');
  ...
  return (
    <div className="px-4 py-5 space-y-6 animate-fade-in-up">
      <div>
        <h1 className="text-2xl font-bold font-display text-foreground">Vibration Therapy 📳</h1>
        <p className="text-sm text-muted-foreground mt-1">Control your wristband's vibration patterns</p>
      </div>
      ...
      <div className="flex items-center gap-3 p-4 bg-card rounded-2xl border border-border">
        ...
        <div>
          ...
          <p className="text-xs text-amber-500">Simulated — BLE not available in browser</p>
        </div>
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-bold text-foreground uppercase tracking-wider">Vibration Pattern</h2>
        {PATTERNS.map(pattern => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-foreground">{pattern.label}</p>
      ...
      <div className="p-4 rounded-2xl bg-primary/5 border border-primary/15">
        <p className="text-xs text-muted-foreground leading-relaxed">
          💡 <strong>How it works:</strong> Vibration therapy uses gentle haptic feedback to help regulate
          your nervous system and reduce stress. Commands are logged for your wellness history.
        </p>
      </div>
    </div>
  );
