...
function computeStressLevel(heartRate: number, skinTemp: number, motion: number): { level: StressLevel; score: number } {
...
function randomFluctuate(value: number, range: number, min: number, max: number): number {
...
export function useStressSimulation(enabled: boolean) {
  ...
  const [data, setData] = useState<SensorData>(() => {
    ...
    return { heartRate: hr, skinTemp: temp, motion, stressLevel: level, stressScore: score };
  });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setData(prev => {
        ...
        const temp = parseFloat(randomFluctuate(prev.skinTemp, 0.15, 35.0, 38.0).toFixed(1));
