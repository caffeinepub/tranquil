...
export function Sounds() {
  const { state, toggleSound, loadLocalTrack, toggleLocalTrack } =
    useAudioPlayer();
    ...
    <div className="px-4 py-5 space-y-6 animate-fade-in-up">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Immerse yourself in soothing audio
        </p>
      </div>
      ...
      <div className="space-y-3">
        ...
        <button
          type="button"
          className="w-full p-5 rounded-2xl border-2 border-dashed border-border bg-card/50
      ...
      <div className="p-4 rounded-2xl bg-primary/5 border border-primary/15">
        <p className="text-xs text-muted-foreground leading-relaxed">
          💡 <strong>Tip:</strong> Combine nature sounds with breathing
          exercises for maximum relaxation effect.
        </p>
      </div>
    </div>
  );
