...
export function useBreathingTimer(technique: BreathingTechnique | null) {
  ...
  const getNextPhase = useCallback(
    (
      ...
      cycle: number,
    ): { phase: BreathPhase; newCycle: number } => {
      if (current === "inhale") {
  ...
  const getPhaseDuration = useCallback(
    (phase: BreathPhase, tech: BreathingTechnique): number => {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      const progress =
        duration > 0 ? Math.min(phaseTimeRef.current / duration, 1) : 1;

      if (phaseTimeRef.current >= duration) {
        const { phase: nextPhase, newCycle } = getNextPhase(
          phaseRef.current,
          technique,
          cycleRef.current,
        );
