...
export function computeDailyAverages(
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const avgScore =
      dayReadings.reduce(
        (sum, r) => sum + stressLevelToScore(r.stressLevel),
        0,
      ) / dayReadings.length;
...
export function computeImprovementMessage(data: DailyStressData[]): string {
  ...
  const recentAvg =
    recent.filter((d) => d.count > 0).reduce((s, d) => s + d.avgScore, 0) /
    (recent.filter((d) => d.count > 0).length || 1);
  const olderAvg =
    older.filter((d) => d.count > 0).reduce((s, d) => s + d.avgScore, 0) /
    (older.filter((d) => d.count > 0).length || 1);
  ...
  if (diff > 5)
    return `You were ${diff}% less stressed recently. Keep it up! 🌟`;
  if (diff < -5)
    return `Stress increased by ${Math.abs(diff)}% recently. Try some breathing exercises. 💙`;
